...
export function EditorHeader(editorHeaderProps: EditorHeaderProps) {
  return (
    <div className="flex flex-grow gap-2 justify-evenly h-1/5 pb-1 transition">
      {Object.entries(cfg).map(([key, value]) => {
        return (
          <button
            key={key}
            className={`${editorHeaderProps.currentSelection === key ? 'border-b-2 border-brand-100 text-brand-100' : ''} h-8 text-sm w-full focus:outline-none`}
            onClick={() => {
              editorHeaderProps.setCurrentSelection(key);
            }}
          >
            {key}
          </button>
